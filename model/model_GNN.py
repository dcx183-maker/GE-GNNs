...
class solvgnn_binary(nn.Module):
    ...
    def forward(self, solvdata, empty_solvsys, gamma_grad=False):
        g1 = solvdata['g1'].to("cpu")
        g2 = solvdata['g2'].to("cpu")
        with g1.local_scope():
            with g2.local_scope():
                
                h1 = g1.ndata['h'].float().cpu()
                h2 = g2.ndata['h'].float().cpu()
                solv1x = solvdata['solv1_x'].float().cpu()
                #solv2x = 1 - solv1x
                solv1x.requires_grad = True
                inter_hb = solvdata['inter_hb'].float().cpu()
                intra_hb1 = solvdata['intra_hb1'].float().cpu()
                intra_hb2 = solvdata['intra_hb2'].float().cpu()
                ...
                hg1 = dgl.mean_nodes(g1, 'h').cpu()
                hg2 = dgl.mean_nodes(g2, 'h').cpu()
